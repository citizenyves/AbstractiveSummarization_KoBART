...
"""
                ! README !
aihub에서 가져온 추가 데이터에 대한 전처리 클래스입니다.
코드출처 : https://dacon.io/competitions/official/235829/codeshare/4047
"""

# read json file 
def read_json(fpath: pathlib.PosixPath) -> List[Dict[str, str]]:
...
class CleanNewspaperArticleBase():
    ...
    def remove_brack_sentence(article_original: List[str]) -> List[str]:
        ...
        bracket_2_pattern = re.compile(r"\s?\(.*\)", re.MULTILINE)      
        bracket_3_pattern = re.compile(r"\s?\[.*\]", re.MULTILINE)      
